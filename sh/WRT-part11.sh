...
# --------------------------------------------------------------------------
# ЗАМЕНА ADGUARD HOME НА ВЕРСИЮ ИЗ MASTER
# (Только если текущая ветка прошивки НЕ master и НЕ main)
# --------------------------------------------------------------------------

# 1. Проверяем ветку: выполняем только для стабильных релизов (не master/main)
if [[ "$REPO_BRANCH" != "master" && "$REPO_BRANCH" != "main" ]]; then

    echo ""
    echo "=================================================="
    echo "Ветка '$REPO_BRANCH': Принудительное обновление AdGuard Home до master..."
    echo "=================================================="

    ADGUARD_PATH="feeds/packages/net/adguardhome"

    # 2. Определяем правильный источник (OpenWrt vs ImmortalWrt)
    # Если URL содержит "git.openwrt.org", значит это официальный OpenWrt
    if [[ "$REPO_URL" == *"git.openwrt.org"* ]]; then
        echo ">>> Обнаружен Official OpenWrt. Источник: github.com/openwrt/packages"
        PKG_REPO_URL="https://github.com/openwrt/packages.git"
    else
        # В остальных случаях считаем, что это ImmortalWrt
        echo ">>> Обнаружен ImmortalWrt. Источник: github.com/immortalwrt/packages"
        PKG_REPO_URL="https://github.com/immortalwrt/packages.git"
    fi

    # 3. Процесс замены пакета
    if [ -d "$ADGUARD_PATH" ]; then
        TEMP_DIR=$(mktemp -d)
        
        echo ">>> Скачивание свежей версии из master..."
        
        # Используем subshell ( ) для изоляции смены директорий
        (
            cd "$TEMP_DIR"
            git init -q
            git remote add origin "$PKG_REPO_URL"
            git config core.sparseCheckout true
            echo "net/adguardhome" >> .git/info/sparse-checkout
            git pull --depth=1 origin master -q
        )

        if [ -d "$TEMP_DIR/net/adguardhome" ]; then
            # Получаем версии для красивого лога
            OLD_VER=$(grep "PKG_VERSION:=" "$ADGUARD_PATH/Makefile" | cut -d'=' -f2 | tr -d ' ')
            NEW_VER=$(grep "PKG_VERSION:=" "$TEMP_DIR/net/adguardhome/Makefile" | cut -d'=' -f2 | tr -d ' ')
            
            echo ">>> Замена файлов..."
            rm -rf "$ADGUARD_PATH"
            cp -r "$TEMP_DIR/net/adguardhome" "$ADGUARD_PATH"
            
            echo "✓ AdGuard Home успешно обновлен: $OLD_VER → $NEW_VER"
        else
            echo "✗ Ошибка: Не удалось скачать пакет из $PKG_REPO_URL"
            # exit 1 # Можно раскомментировать, если хотите, чтобы сборка падала при ошибке
        fi
        
        rm -rf "$TEMP_DIR"
    else
        echo "⚠ Папка $ADGUARD_PATH не найдена в feeds. Пропускаем."
    fi

else
    echo ""
    echo "=================================================="
    echo "Ветка '$REPO_BRANCH' (master/main). Используем штатный AdGuard Home."
    echo "=================================================="
fi
